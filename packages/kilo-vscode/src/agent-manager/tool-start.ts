...
async function worktree(
  ...
  const base = branch(task.branchName) ?? branch(task.name)
  const text = label(task.name) ?? label(task.branchName) ?? label(task.prompt)
  const version = versionedName(base, versions ? index : 0, versions ? total : 1)
  const created = await deps.createWorktree({
    groupId,
    branchName: version.branch,
    name: version.branch,
    label: versionedLabel(text, versions ? index : 0, versions ? total : 1),
  })
