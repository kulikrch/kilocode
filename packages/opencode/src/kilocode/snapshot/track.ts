...
export namespace KiloSnapshotTrack {
  ...
  export interface ProtectInput<A, R = never> {
    readonly inner: Effect.Effect<A, never, R>
   ...
   */
  export const protect = <A, R = never>(input: ProtectInput<A, R>): Effect.Effect<A, never, R> =>
    Effect.gen(function* () {
  ...
  export interface WrapInput<R = never> {
    readonly inner: Effect.Effect<string | undefined, never, R>
    readonly state: State
   ...
   */
  export const wrap = <R = never>(input: WrapInput<R>): Effect.Effect<string | undefined, never, R> =>
    Effect.gen(function* () {
  ...
  export const defaultHooks: Hooks = {
    ...
    async persistDisable() {
      ...
      if (!directory) return
      const root = directory.worktree ?? directory.directory
      const file = path.join(root, ".kilo", "kilo.json")
      ...
      await fsRt.runPromise((fs) =>
        Effect.gen(function* () {
          ...
          const existing = ConfigParse.jsonc(before, file) as Config.Info
