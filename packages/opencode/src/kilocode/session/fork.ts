...
export function writer(sessionID: SessionID, sync: { run: typeof SyncEvent.run }) {
  const items: Item[] = []
  return {
          ...
          () => {
            ...
            for (const item of items) {
              if (item.type === "message") {
                sync.run(MessageV2.Event.Updated, { sessionID, info: item.info }, { publish: false })
                continue
              }
              sync.run(MessageV2.Event.PartUpdated, { sessionID, part: item.part, time: item.time }, { publish: false })
            }
          },
          { behavior: "immediate" },
        ),
