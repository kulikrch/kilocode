...
export namespace KiloCompactionPayloadRecovery {
  ...
  export function prompt(text: string) {
    return [
      "The previous compaction request exceeded the provider payload limit.",
  ...
  export function process(input: {
    ...
    return run(input.messages, input.prompt).pipe(
      Effect.flatMap((result) => {
        ...
        if (result === "compact" && !matches(input.processor.compactError?.())) return Effect.succeed(result)
        return Effect.gen(function* () {
          ...
          const stripped = yield* MessageV2.toModelMessagesEffect(input.recovery, input.model, { stripMedia: true })
