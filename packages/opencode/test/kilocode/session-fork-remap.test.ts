  ...
  test(
    ...
    async () => {
      ...
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          const original = await Session.create({ title: "original" })
          ...
          const source = await Session.messages({ sessionID: original.id })
          const copy = await Session.messages({ sessionID: forked.id })
    ...
    async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          await Session.updatePart(taskPart({ messageID: assistant, sessionID: parent.id, childSessionID: child.id }))
          const before = await Array.fromAsync(Session.list())
          const forked = await Session.fork({ sessionID: parent.id })

          expect(await Array.fromAsync(Session.list())).toHaveLength(before.length + 1)
          ...
          const source = await Session.messages({ sessionID: parent.id })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          await Session.updatePart({
            ...
            sessionID: parent.id,
            type: "text",
            text: "Initializing snapshot... transient",
            synthetic: true,
          ...
          await Session.updatePart({
            ...
            sessionID: parent.id,
            type: "text",
            text: "Initializing snapshot... but durable",
            synthetic: true,
          } as MessageV2.TextPart)
          await Session.updatePart({
            id: PartID.ascending(),
            ...
            sessionID: parent.id,
            type: "text",
            text: "<system-reminder>durable context</system-reminder>",
            metadata: { source: "system-reminder" },
          } as MessageV2.TextPart)

          const forked = await Session.fork({ sessionID: parent.id })
          const msgs = await Session.messages({ sessionID: forked.id })
          const texts = msgs.flatMap((msg) => msg.parts).map((part) => (part.type === "text" ? part.text : ""))
